      ...
      <Card className={classes.root} elevation={1}>
        <CardContent className={classes.cardContent}>
          ...
          <div className={classes.detailSection}>
            ...
            <div>
              <Typography gutterBottom={width === 'xs'? true : false} variant="caption" className={width === 'xs'? classes.labelBlock : classes.labelInline}>
                {venue}
              </Typography>
              {tags.map((tag) => (
                <SessionTag key={tag.id} label={tag.label} color={tag.color || ''}/>
              ))}
            </div>
          </div>
          <IconButton className={classes.favButton} aria-label="Favourite" onClick={favClick}>
            <Icon>star</Icon>
          </IconButton>
        </CardContent>
      </Card>
